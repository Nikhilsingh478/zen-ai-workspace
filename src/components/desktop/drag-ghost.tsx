...
export function DragGhost({ entry }: DragGhostProps) {
  if (entry.kind === "item") {
    ...
    return (
      <div className="flex flex-col items-center justify-center gap-2 rounded-2xl p-2 w-full h-full bg-white/[0.10] ring-1 ring-white/25 shadow-[0_8px_32px_rgba(0,0,0,0.8)] rotate-1 scale-105">
        ...
        <span className="text-[11px] text-white/90 text-center leading-tight w-full truncate px-1">
          {label}
        </span>
      </div>
    );
