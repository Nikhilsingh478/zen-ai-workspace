...
export function DragGhost({ entry }: DragGhostProps) {
  if (entry.kind === "item") {
    ...
    const label = isWebsite ? item.name : "";
    const url = isWebsite ? item.url : undefined;

    return (
      <div className="flex flex-col items-center gap-2 rounded-2xl p-3 bg-white/[0.06] ring-1 ring-white/20 shadow-[0_8px_32px_rgba(0,0,0,0.6)]">
        <div className="h-14 w-14 rounded-2xl overflow-hidden bg-[#18181B] border border-white/20 flex items-center justify-center shadow-lg">
          {isWebsite && url ? (
            <img
        ...
        <span className="text-[11px] text-white/80 text-center leading-tight max-w-[80px] truncate">
          {label}
        </span>
      </div>
    );
